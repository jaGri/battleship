...
async fn test_version_mismatch_reject() {
    ...
    assert!(matches!(msg, WireMessage::Handshake { version } if version == PROTOCOL_VERSION));

    // Player 2 responds with incompatible version
    let wrong_version = PROTOCOL_VERSION + 10;
    t2.send(WireMessage::HandshakeAck {
    ...
    if let WireMessage::HandshakeAck { version } = reply {
...
async fn test_old_client_new_server() {
    ...
    assert!(matches!(msg, WireMessage::Handshake { version } if version == old_version));
...
async fn test_future_version() {
    ...
    let msg = t2.recv().await.unwrap();
    assert!(matches!(msg, WireMessage::Handshake { version } if version == future_version));
...
async fn test_version_zero() {
    ...
    t1.send(WireMessage::Handshake { version: 0 })
        .await
        .unwrap();

    let msg = t2.recv().await.unwrap();
    assert!(matches!(msg, WireMessage::Handshake { version: 0 }));
...
async fn test_multiple_version_handshakes() -> anyhow::Result<()> {
    ...
    let server_task = tokio::spawn(async move {
        ...
        for _ in 0..3 {
            let msg = transport.recv().await.unwrap();
            if let WireMessage::Handshake { version: _ } = msg {
    ...
    for offset in 0..3 {
        ...
        transport.send(WireMessage::Handshake { version }).await?;

        let reply = transport.recv().await?;
        if let WireMessage::HandshakeAck {
            version: ack_version,
        } = reply
        {
...
async fn test_version_in_guess_message() {
    ...
    let msg = t2.recv().await.unwrap();
    if let WireMessage::Guess { version, .. } = msg {
        assert_eq!(version, wrong_version);
        assert_ne!(version, PROTOCOL_VERSION);
    }
}
...
async fn test_compatible_versions() {
    ...
    assert!(
        matches!(guess_msg, WireMessage::Guess { version, seq: 0, x: 5, y: 5 } if version == PROTOCOL_VERSION)
    );
}
