...
async fn test_malformed_length_prefix() -> anyhow::Result<()> {
    ...
    assert!(
        err_msg.contains("too large")
            ...
            || err_msg.contains("WireMessage size")
    );
...
async fn test_malformed_message_type() -> anyhow::Result<()> {
    ...
    let server_task = tokio::spawn(async move {
        ...
        // Create a WireMessage with invalid enum variant
...
async fn test_valid_length_but_wrong_structure() -> anyhow::Result<()> {
    ...
    let server_task = tokio::spawn(async move {
        ...
        // Send a valid-looking length with an enum variant index that no WireMessage has.
