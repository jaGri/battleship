...
use battleship::{
    ...
    HeartbeatTransport, TcpTransport,
};
...
async fn test_in_memory_closed_channel_detection() {
    ...
    // Send a WireMessage from t1 to t2
    t1.send(WireMessage::Heartbeat {
    ...
    // t2 should receive the WireMessage that was already sent
...
async fn test_tcp_bounded_message_size() {
    ...
    let server_task = tokio::spawn(async move {
        ...
        let mut transport = TcpTransport::with_config(
            ...
            1000, // Very small max WireMessage size
            Duration::from_secs(10),
            Duration::from_secs(30),
        );

        // Try to receive - will fail if client sends large WireMessage
        let result = transport.recv().await;
        if let Err(e) = result {
    ...
    let client_task = tokio::spawn(async move {
        ...
        // Try to send a WireMessage with a large guess sequence
        // This should work on the client side but fail on server due to size limit
        transport
            .send(WireMessage::Guess {
                version: PROTOCOL_VERSION,
                seq: 12345,
                x: 5,
                y: 5,
...
async fn test_tcp_heartbeat() {
    ...
    let server_task = tokio::spawn(async move {
        ...
        let mut transport =
            HeartbeatTransport::new(tcp, Duration::from_millis(100), Duration::from_secs(5));

        // Send a game WireMessage
        transport
        ...
        // Receive game WireMessage (heartbeats are filtered)
        let msg = transport.recv().await.unwrap();
        assert!(matches!(msg, WireMessage::Handshake { .. }));
    });
...
async fn test_heartbeat_multiplexing() {
    ...
    let server_task = tokio::spawn(async move {
        ...
        let tcp = TcpTransport::new(stream);
        let mut transport =
            HeartbeatTransport::new(tcp, Duration::from_millis(50), Duration::from_secs(5));

        // Send multiple game messages while heartbeats run in background
        for i in 0..5 {
            transport
                .send(WireMessage::Handshake { version: i })
                .await
                .unwrap();
        ...
        for i in 0..5 {
            let msg = transport.recv().await.unwrap();
            assert!(matches!(msg, WireMessage::HandshakeAck { version } if version == i));
        }
    });
    ...
    let client_task = tokio::spawn(async move {
        ...
        for i in 0..5 {
            let msg = transport.recv().await.unwrap();
            assert!(matches!(msg, WireMessage::Handshake { version } if version == i));
            transport
                .send(WireMessage::HandshakeAck { version: i })
                .await
                .unwrap();
        }
    });
...
async fn test_heartbeat_idle_timeout() {
    ...
    let server_task = tokio::spawn(async move {
        ...
        let mut transport =
            HeartbeatTransport::new(tcp, Duration::from_secs(100), Duration::from_millis(200));
        ...
        let err_msg = result.unwrap_err().to_string();
        assert!(
            err_msg.contains("idle") || err_msg.contains("timeout"),
...
async fn test_heartbeat_disabled() {
    ...
    let server_task = tokio::spawn(async move {
        ...
        // Receive response
        let msg = transport.recv().await.unwrap();
        assert!(matches!(msg, WireMessage::HandshakeAck { .. }));
    });
